...
def _render_script_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            st.caption(tr("Channel Simulation Help"))
            channel_source_url = st.text_input(
                tr("YouTube Video or Channel URL"),
                placeholder=tr("YouTube URL Placeholder"),
                key="channel_source_url",
            ).strip()
            if st.button(
                tr("Simulate Channel Style"),
                key="simulate_channel_style",
                use_container_width=True,
                type="secondary",
                icon=":material/auto_awesome:",
            ):
                if not channel_source_url:
                    st.warning(tr("Please Enter a YouTube URL"))
                else:
                    try:
                        with st.spinner(tr("Fetching Transcript and Generating Script")):
                            simulated_script, source_video_id = (
                                channel_simulator.simulate_from_source(
                                    channel_source_url,
                                    language=params.video_language,
                                    paragraph_number=params.paragraph_number,
                                )
                            )
                        st.session_state["video_script"] = simulated_script
                        st.session_state["channel_simulation_source_video_id"] = (
                            source_video_id
                        )
                        st.success(tr("Channel Simulation Completed"))
                    except channel_simulator.ChannelSimulationError as exc:
                        st.error(str(exc))
